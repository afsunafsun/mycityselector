...
class MycityselectorModelFields extends Joomla\CMS\MVC\Model\ListModel
{
	...
	public function getItem($id = 0)
	{
		...
		foreach ($data['fieldValues'] as $k => $fieldValue)
		{
			...
			$query  = "SELECT `city`.`id`, `city`.`name` FROM `{$this->table_valuecities}` `t` "
				. "INNER JOIN `#__mycityselector_city` `city` ON `t`.`city_id` = `city`.`id` WHERE `t`.`field_value_id` = {$fid}";
	...
	public function saveItem($data)
	{
		...
		if (!empty($data[$prefix]))
		{
			...
			{
				// create
				if ($this->checkLimitsReached())  return 0;
				$result = $this->_db
	...
	private function saveFieldValues($fieldId, $data)
	{
		foreach ($data['value'] as $key => $value)
		{
				...
				if ($result)
				{
					...
					if (!empty($cities))
					{
						foreach ($cities as $cityId)
						{
							...
							$this->_db->setQuery("INSERT INTO `{$this->table_valuecities}` (`field_value_id`,`city_id`) "
			...
			else
			{
				...
				// - cities
				$this->_db->setQuery("DELETE FROM `{$this->table_valuecities}` WHERE `field_value_id` = {$id}")->execute(); // remove all links
				if (!empty($cities))
				{
					foreach ($cities as $cityId)
					{
	...
	public function publishItems($keys, $status = '1')
	{
		...
		if ($status == 1 && $this->checkLimitsReached()) {
            return false;
		}
	...
	public function deleteFieldValue($id)
	{
		...
		else
		{
			...
			$query = $this->_db->getQuery(true)->delete($this->table_valuecities)->where('field_value_id=' . $this->_db->quote($id));
			$this->_db->setQuery($query)->execute();
	...
	private function checkLimitsReached()
	{
	    if (McsData::MCS_FREE == true) {
            ...
            if ($isExists >= McsData::MCS_LIMIT_15) {
                return true;
            }
        }
        return false;
